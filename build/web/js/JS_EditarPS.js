...
const productos = {
    ...
    "CAOMIN C08-20": {
        expresiones: {
            ...
            // Brigh: /^(?:77(?:\.\d{1,2})?|7[8-9](?:\.\d{1,2})?|8[0-2](?:\.\d{1,2})?|83(?:\.0{1,2})?)$/, //Rango 77-83
            //Whitness: /^(?:6[1-9](?:\.\d{1,2})?|70(?:\.0{1,2})?)$/, //Rango 61-70.0
            retenido325: /^(?:0(?:\.[0-5]\d{0,2}?|\.60?)|1(?:\.\d{0,2}?)|2(?:\.\d{0,2}?)|3(?:\.\d{0,2}?)|4(?:\.\d{0,2}?)|5(?:\.\d{0,2}?)|6(?:\.0{0,2}))$/, // Rango Max 6.0
...
let productoSeleccionado = "CAOMIN P055";

const validarCampoSeco = (expReg, input, campo, nombreProducto) => {
    const grupo = input.parentElement;
    const iconoExito = input.nextElementSibling;
    const iconoError = input.nextElementSibling ? input.nextElementSibling.nextElementSibling : null;
...
const validarFormularioEditarS = (e) => {
    ...
    if (expReg) {
        validarCampoSeco(expReg, e.target, nombreCampo, nombreProducto);
    }
};

inputs.forEach((input) => {
    input.addEventListener('keyup', validarFormularioEditarS);
    input.addEventListener('blur', validarFormularioEditarS);
});
...
document.getElementById('formulario').addEventListener('submit', function (event) {
    ...
    if (validarCamposVacios() && validarFormularioPSeco()) {
...
function validarFormularioPSeco() {
    var camposObligatorios = [
        "idUsuarios", "fecha", "producto"
    ];
    var camposValidos = true;
    for (var i = 0; i < camposObligatorios.length; i++) {
        ...
        if (input.tagName.toLowerCase() === "select") {
            if (!validarCampoComboPSeco(input)) {
                camposValidos = false;
            }
        } else {
            validarCampoPSeco(input);
